...
class UnsplashImageService {
  ...
  /**
   * 키워드 매핑 설정 로드 (site.config.js에서)
   */
  loadKeywordMappings() {
    try {
      const siteConfig = require('../../config/site.config.js');
      this.keywordConfig = siteConfig.blogTheme.imageKeywords;
      this.logger?.info('이미지 키워드 설정 로드 완료 (site.config.js)');
    } catch (error) {
      this.logger?.error('치명적 오류: site.config.js 로드 실패', { error: error.message });
      // site.config.js 로드 실패는 치명적 오류 - 시스템 중단
      throw new Error(`이미지 서비스 초기화 실패: ${error.message}`);
    }
  }
  ...
  extractKeywordsFromText(text) {
    ...
    // 2. 허용된 영어 키워드만 추출 (통제된 방식)
    const allowedKeywords = this.keywordConfig.allowedEnglishKeywords || [];
    const foundKeywords = allowedKeywords.filter(keyword => 
      lowerText.includes(keyword.toLowerCase())
    );
    keywords.push(...foundKeywords);

    // 3. 키워드가 있으면 중복 제거 후 반환, 없으면 기본값 사용
    if (keywords.length > 0) {
      const uniqueKeywords = [...new Set(keywords)];
      return uniqueKeywords.slice(0, 5);
    }

    // 4. 매칭되는 키워드가 없으면 기본값 사용 (site.config.js에서)
    const defaultKeywords = this.keywordConfig.defaultKeywords;
    this.logger?.info(`키워드 매칭 실패, 기본 키워드 사용: ${defaultKeywords.join(', ')}`);
