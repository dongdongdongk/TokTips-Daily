...
class MetadataGenerator {
  ...
  generateMetadata(aiData, title, settings, mainImage, aiGeneratedTags = []) {
    ...
    if (aiGeneratedTags.length > 0) {
      ...
      // AI 태그가 없으면 설정 파일의 기본 태그 사용
      const defaultTags = config.postProcessing?.tags?.defaultTags || ["TikTok", "Social Media"];
      const additionalTags = this.extractTagsFromTitle(title);
      allTags = [...new Set([...defaultTags, ...additionalTags])];
    }
    ...
    return {
      ...
      author: config.branding?.author || config.content?.defaultAuthor || "WebMaker Team",
